...
digital data communication channels,
...
def decodeMorse(code_Morse):
    ...
    inv_dict_Morse = dict(zip(dict_Morse.values(), dict_Morse.keys()))
    
    list_words = [i.split() for i in code_Morse.strip().split('   ')]
    
    result = []
    for item in list_words:
        word = ''
        for letter in item:
            word += inv_dict_Morse[letter]
        result.append(word)
    
    return ' '.join(result)


print(decodeMorse(code_Morse))
